...
class InquiryFormController extends Controller
{
    ...
    public function export(Request $request)
    {
        ...
        // フルネーム検索（姓+名をスペース無視で検索）
        $name = $request->input('keyword');
        $trimmedName = str_replace([' ', '　'], '', trim($name)); // trim + 半角・全角スペース除去

        if (!empty($trimmedName)) {
            $query->where(DB::raw("REPLACE(REPLACE(CONCAT(IFNULL(last_name, ''), IFNULL(first_name, '')), ' ', ''), '　', '')"), 'like', '%' . $trimmedName . '%');
        }
